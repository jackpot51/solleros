...
public class unfs {
    ...
    public static void writeHeader(String t, int v, int bs, int ms, int us, int cs, int ce, String rn){
	try {
            br.write("fsys:\n");
    ...
    public static void writeFolderIndexes(File folder){
        try{
            ...
            String fnamei;
            if(folder.getParent() != null){
                fnamei = folder.getParent() + "." + folder.getName();
            }else{
                fnamei = folder.getName();
            }
            String fname = fnamei.replace('\\', '.').replace('/','.');
            for(int i = 0; i<files.length; i++){
                String name = fname + "." + files[i].getName();
